...
const OfferAccepted = ({ navigation }) => {
  ...
  const fetchOfferOrders = async () => {
    try {
      ...
      const response = await axios.get('http://176.119.254.188:8080/customer/offer/orders/accepted', {
        headers: {
  ...
  const submitOrder = async () => {
    try {
      ...
      const response = await axios.post(
        'http://176.119.254.188:8080/customer/offer/order/submit',
      ...
      if (response.status === 200) {
        ...
        // Optionally, you can refresh the list of orders or navigate to a different screen
        fetchOfferOrders();
      }
    } catch (error) {
  ...
  return (
    ...
    <ScrollView style={{ flex: 1 }}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerText}>Offers Accepted Orders</Text>
      </View>

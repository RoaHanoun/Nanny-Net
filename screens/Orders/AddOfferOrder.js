...
const AddOfferOrder = ({ route, navigation }) => {
  ...
  const cities = [
    ...
    { label: 'Beit Sahour', value: 'Beit Sahour' }, // Added city for order data
  ];
  ...
  const handleOrder = async () => {
    try {
      ...
      const orderData = {
        ...
        startTime: `${timeFrom} ${periodFrom}`,
        endTime: `${timeTo} ${periodTo}`,
        description: additionalDesc,
        numOfKids: numberOfChildren,
        location: {
          city: locationCity,
          streetData: locationStreet,
          extraDescription: locationDesc || "",
        },
      };
      ...
      const response = await axios.post(
        'http://176.119.254.188:8080/customer/offer/order/request',
        orderData,
        {
          headers: {
      ...
      if (response.status === 200) {
        navigation.navigate('Conformationoffer', {
          orderData,
        });
      } else {

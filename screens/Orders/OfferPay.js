...
const OfferPay = ({ route, navigation }) => {
  ...
  const handleMakeRequest = async () => {
    try {
      ...
      // Send a GET request to the API with the offer ID and JWT token in the headers
      const response = await axios.get(
        `http://176.119.254.188:8080/customer/offer/submit`,
      ...
      if (response.status === 200) {
        ...
        navigation.navigate('OfferConfirmation', { data }); // Pass data to the next screen
      }
    } catch (error) {
